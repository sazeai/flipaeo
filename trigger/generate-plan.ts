 ...
 * 3. This task runs: SERP → Gap → Hierarchy → Plan Generation
...
export const generatePlanTask = task({
    ...
    run: async (payload: GeneratePlanPayload) => {
        const { planId, userId, brandId, brandData, seeds, competitorBrands: passedCompetitorBrands, existingContent } = payload
